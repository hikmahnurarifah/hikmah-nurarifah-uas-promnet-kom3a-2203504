...
func GetUsers(w http.ResponseWriter, r *http.Request) {
	...
	result, err := db.Query("SELECT id, " +
		"nama,usia,jenis_kelamin,alamat,deskripsi from pasien_puskesmas_hikmah_nurarifah")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
	for result.Next() {
		...
		err := result.Scan(&user.ID,
			&user.Nama, &user.Usia, &user.JenisKelamin, &user.Alamat, &user.Deskripsi)
		if err != nil {
			panic(err.Error())
		}
...
func CreateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := db.Prepare("INSERT INTO pasien_puskesmas_hikmah_nurarifah(nama," +
...
func GetUser(w http.ResponseWriter, r *http.Request) {
	...
	params := mux.Vars(r)
	result, err := db.Query("SELECT id, "+
		"nama,usia,jenis_kelamin,alamat,deskripsi from pasien_puskesmas_hikmah_nurarifah WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateUser(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := db.Prepare("UPDATE pasien_puskesmas_hikmah_nurarifah SET nama = ?," +
...
func DeleteUser(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := db.Prepare("DELETE FROM pasien_puskesmas_hikmah_nurarifah WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func InitDB() {
	db, err = sql.Open("mysql",
		"root:@tcp(127.0.0.1:3306)/DB_2203504_Hikmah_Nurarifah_UAS")
	if err != nil {
		panic(err.Error())
	}
}
